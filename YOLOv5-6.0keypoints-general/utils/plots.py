...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    ...
    blank = np.full((int(ns * h), int(ns * w), 3), 255, dtype=np.uint8)  # init
        ...
        if len(targets) > 0:
            ...
            for j, pt in enumerate(pts.T):
                ...
                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    ...
                    plot_one_box(pt, mosaic, label=label, color=color, line_thickness=tl)
...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    for k in range((len(x)//2)-1):
        cv2.line(img, (int(x[2*k]), int(x[2*k+1])), (int(x[2*k+2]), int(x[2*k+3])), color, thickness=tl, lineType=cv2.LINE_AA)
    cv2.line(img, (int(x[-2]), int(x[-1])), (int(x[0]), int(x[1])), color, thickness=tl, lineType=cv2.LINE_AA)
