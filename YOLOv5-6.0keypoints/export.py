...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
    ...
    im = torch.zeros(batch_size, *imgsz, 3).to(device)  # image size(1,3,320,192) BCHW iDetection
    #################################
    # Update model
    if half:
    ...
    model = Export_Input(model)
    model = Export_OnlyOutput(model)
    ##################################
    for _ in range(2):
        y = model(im)  # dry runs
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default='/cluster/home/it_stu3/Data/4-9homo/armor.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', type=str, default='runs/train/2525/weights/best-cut.pt', help='weights path')
